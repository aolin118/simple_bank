...
func (server *Server) createTransfer(ctx *gin.Context) {
	...
	fromAccount, ok := server.validateAccount(ctx, req.FromAccountID, req.Currency)

	if !ok {
		return
	}

	authTokenPayload := ctx.MustGet(authorizationPayloadKey).(*token.Payload)
	if authTokenPayload.Username != fromAccount.Owner {
